<script>
    $(document).ready(function(){
        ...
        $('#submit-upload').on('click', function () {
            ...
            if (!file && !urlInput) {
                console.log($('#error-text').text('Please choose your package file or enter url Input.'));
                ...
                if(file)
                {
                    ...
                    if (!allowedExtensions.includes(extension)) {
                        $('#error-text').text('Only .zip files are allowed.');
                        check_error = true;
                    } else {
                        // Check file size
                        if (file.size > 20 * 1024 * 1024) { // 5MB in bytes
                            $('#error-text').text('File size should be less than 20MB.');
                            check_error = true;
                        }
                    }
                }
            ...
            if (!check_error) {
                M.Modal.getInstance($('#upload_package')).close();
                ...
                $('#modal-text').html('');
                M.Modal.getInstance($('#staticBackdrop')).open();
                ...
                $.ajax({
                    ...
                    complete: function (xhr_unzip_solution, status_unzip_solution) {
                        ...
                        $('.modal').scrollTop($('#modal-text').height());
                                                            ...
                                                            if (response_generate_data_structure.status == 'success') {
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    complete: function (xhr_composer_update, status_composer_update) {
                                                                        ...
                                                                        $('#modal-text').html(modalText);
                                                                        $('.modal').scrollTop($('#modal-text').height());
                                                                        $('.modal-footer').html(`<button id="modal-close" type="button" class="btn btn-secondary modal-close" data-bs-dismiss="modal">Close</button>`);
                                                                    }
                                                                });
                                                            } else {
                                                                $('.progress').css("display", "none");
